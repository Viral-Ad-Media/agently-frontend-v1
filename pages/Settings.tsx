...
const TIMEZONES = [
  ...
  'Australia/Sydney','Africa/Lagos','Africa/Nairobi','Africa/Johannesburg',
];
...
const Settings: React.FC<SettingsProps> = ({ org, onSave }) => {
  ...
  return (
    <div className="space-y-6 animate-fade-up">
      ...
      {message && <div className="rounded-2xl border border-emerald-100 bg-emerald-50 px-4 py-3 text-sm font-medium text-emerald-700 flex items-center gap-2"><i className="fa-sharp fa-solid fa-circle-check" />  {message}</div>}
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="bg-white rounded-3xl border border-slate-200 shadow-sm p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5">Organization Name</label>
            <input type="text" value={org.profile.name} disabled
          ...
          <div>
            ...
            <select value={timezone} onChange={e => setTimezone(e.target.value)}
              ...
              {[timezone, ...TIMEZONES.filter(v => v !== timezone)].map(v => <option key={v} value={v}>{v}</option>)}
            </select>
          </div>
          <div>
            <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5">Primary Contact Number</label>
            <input value={phoneNumber} onChange={e => setPhoneNumber(e.target.value)} placeholder="+1 555 000 0000"
              className="w-full px-4 py-2.5 rounded-xl border border-slate-200 bg-white font-medium text-sm outline-none focus:ring-2 focus:ring-amber-400 transition-all" />
          </div>
          <div>
            <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5">Industry</label>
            <input value={org.profile.industry} disabled
              className="w-full px-4 py-2.5 rounded-xl border border-slate-100 font-medium bg-slate-50 text-slate-500 text-sm" />
          </div>
        </div>
        ...
        <div className="flex justify-end">
          <button type="submit" disabled={saving}
            className="rounded-2xl bg-slate-900 text-white px-8 py-3 text-[10px] font-black uppercase tracking-widest hover:bg-amber-600 disabled:opacity-50 transition-all active:scale-95 flex items-center gap-2">
            {saving ? <><i className="fa-sharp fa-solid fa-spinner fa-spin" /> Saving…</> : <><i className="fa-sharp fa-solid fa-floppy-disk" /> Save Settings</>}
          </button>
        </div>
      </form>
    </div>
