    ...
    <div className="animate-fade-up space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-0.5">Search, purchase and assign phone numbers to your voice agents</p>
        </div>
        ...
        <div className="space-y-5">
          <div className="bg-white rounded-3xl border border-slate-200 shadow-card p-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-5">
              <div>
                <Label>Country</Label>
                <Sel value={country} onChange={e => { setCountry(e.target.value); setNumberType('Local'); setSearchResults([]); setSearchDone(false); }}>
                  {countries.length > 0
                    ? countries.map(c => (
              ...
              <div>
                ...
                <Sel value={numberType} onChange={e => setNumberType(e.target.value as any)}>
                  ...
                  {(() => {
                    ...
                    // If no country data loaded yet, show all; otherwise filter
                    if (!c) return <><option value="TollFree">Toll Free</option><option value="Mobile">Mobile</option></>;
                    return <>
                      {c.hasTollFree && <option value="TollFree">Toll Free</option>}
      ...
      {tab === 'owned' && (
        <div className="bg-white rounded-3xl border border-slate-200 shadow-card p-6">
          <div className="flex items-center justify-between mb-5">
            <h3 className="text-base font-black text-slate-900">Your Numbers</h3>
          ...
          ) : ownedNumbers.filter(n => org.voiceAgents.some(a => a.twilioPhoneNumber === n.phoneNumber)).length === 0 && ownedNumbers.length > 0 ? (
            <div className="rounded-2xl border-2 border-dashed border-slate-200 py-12 text-center">
              <div className="text-3xl mb-3 text-slate-300">
                <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="mx-auto"><rect width="14" height="20" x="5" y="2" rx="2"/><path d="M12 18h.01"/></svg>
              </div>
              <p className="text-sm font-bold text-slate-400">No numbers assigned to your agents yet.</p>
              <p className="text-xs text-slate-300 mt-1">Purchase one from the "Get a Number" tab.</p>
            </div>
          ) : ownedNumbers.length === 0 ? (
            <div className="rounded-2xl border-2 border-dashed border-slate-200 py-12 text-center">
              <p className="text-sm font-bold text-slate-400">No numbers found.</p>
              <p className="text-xs text-slate-300 mt-1">Purchase one from the "Get a Number" tab.</p>
            </div>
          ) : (
            <div className="space-y-3">
              {ownedNumbers
                .filter(num => org.voiceAgents.some(a => a.twilioPhoneNumber === num.phoneNumber))
                .map(num => {
      ...
      <div className="bg-slate-50 border border-slate-200 rounded-2xl p-5 flex gap-3">
        ...
        <p className="text-xs text-slate-500 leading-relaxed">
          Numbers purchased here are assigned directly to your voice agents. Once assigned, inbound calls to that number are automatically handled by your configured AI agent.
        </p>
      </div>
    </div>
  );
