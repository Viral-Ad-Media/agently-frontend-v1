...
const PhoneNumbers: React.FC<PhoneNumbersProps> = ({ org, onAgentUpdated }) => {
  ...
  const TABS: { id: Tab; label: string; faIcon: string }[] = [
    {
      id: "assigned",
      label: "Agent Numbers",
      faIcon: "fa-sharp fa-solid fa-mobile-screen",
    },
    {
      id: "search",
      label: "Get a Number",
      faIcon: "fa-sharp fa-solid fa-magnifying-glass",
    },
    {
      id: "owned",
      label: "All Owned",
      faIcon: "fa-sharp fa-solid fa-layer-group",
    },
  ];
    ...
    <div className="animate-fade-up space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-0.5">
            Search, purchase and assign phone numbers to your voice agents
          </p>
        </div>
        ...
        <div className="space-y-4">
          {org.voiceAgents.map((agent) => {
            ...
            return (
              <div
                key={agent.id}
                className={`bg-white rounded-3xl border-2 shadow-sm p-6 ${isActive ? "border-amber-300" : "border-slate-100"}`}
              >
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
                  <div className="flex items-center gap-3">
                    <div
                      className={`w-10 h-10 rounded-2xl flex items-center justify-center ${isActive ? "bg-amber-100 text-amber-600" : "bg-slate-100 text-slate-500"}`}
                    >
                      <i
                        className={`fa-sharp fa-solid ${agent.direction === "inbound" ? "fa-phone-arrow-down-left" : "fa-phone-arrow-up-right"} text-base`}
                      />
                    </div>
                    <div>
                ...
                {agent.twilioPhoneNumber ? (
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center gap-3 bg-slate-50 border-2 border-dashed border-slate-200 rounded-2xl p-4">
                    <div className="w-10 h-10 rounded-2xl bg-slate-200 flex items-center justify-center text-slate-400">
                      <i className="fa-sharp fa-solid fa-phone-slash text-base" />
                    </div>
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-slate-400 mt-0.5">
                        Go to "Get a Number" tab to search and purchase one
                      </p>
                    </div>
                    <button
                      onClick={() => {
          ...
          <div className="bg-white rounded-3xl border border-slate-200 shadow-sm p-6">
                  ...
                  {(() => {
                    ...
                    return (
                      <>
                        ...
                        {c.hasMobile && <option value="Mobile">Mobile</option>}
                      </>
                    );
                  })()}
                </Sel>
            ...
            >
              {busy === "search" ? (
                ...
                <>
                  <i className="fa-sharp fa-solid fa-magnifying-glass text-xs" />{" "}
                  Search Numbers
                </>
              )}
            </button>
            ...
            <div className="bg-white rounded-3xl border border-slate-200 shadow-sm p-6">
              ...
              {searchResults.length === 0 ? (
                <div className="rounded-2xl border-2 border-dashed border-slate-200 py-12 text-center">
                  <i className="fa-sharp fa-solid fa-telescope text-3xl text-slate-300 mb-3 block" />
      ...
      {tab === "owned" && (
        <div className="bg-white rounded-3xl border border-slate-200 shadow-sm p-6">
          <div className="flex items-center justify-between mb-5">
            ...
            >
              <i className="fa-sharp fa-solid fa-rotate text-xs" /> Refresh
            </button>
          </div>
          ...
          {busy === "owned" ? (
            ...
            <div className="rounded-2xl border-2 border-dashed border-slate-200 py-12 text-center">
              <i className="fa-sharp fa-solid fa-sim-card text-3xl text-slate-300 mb-3 block" />
              <p className="text-sm font-bold text-slate-400">
                No numbers found.
              </p>
              <p className="text-xs text-slate-300 mt-1">
                Purchase one from the "Get a Number" tab.
              </p>
            </div>
          ) : (
      ...
      <div className="bg-slate-50 border border-slate-200 rounded-2xl p-5 flex gap-3">
        <div className="text-slate-400 shrink-0 mt-0.5">
          <i className="fa-sharp fa-solid fa-circle-info text-base" />
        </div>
        <p className="text-xs text-slate-500 leading-relaxed">
          Numbers purchased here are assigned directly to your voice agents.
          Once assigned, inbound calls to that number are automatically handled
          by your configured AI agent.
        </p>
      </div>
    </div>
  );
