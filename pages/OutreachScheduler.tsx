...
const FALLBACK_TIMEZONES = [
  ...
  "America/Detroit", "America/Halifax", "America/Los_Angeles", "America/Mexico_City", "America/New_York",
  "America/Phoenix", "America/Sao_Paulo", "America/Toronto", "America/Vancouver",
  "Asia/Bangkok", "Asia/Dubai", "Asia/Hong_Kong", "Asia/Karachi", "Asia/Kolkata", "Asia/Riyadh",
  "Asia/Shanghai", "Asia/Singapore", "Asia/Seoul", "Asia/Tokyo",
  "Australia/Melbourne", "Australia/Perth", "Australia/Sydney",
  "Europe/Amsterdam", "Europe/Athens", "Europe/Berlin", "Europe/Lisbon", "Europe/London", "Europe/Madrid",
...
const TimeListEditor = memo(({
  ...
  const remove = (index: number) => {
    onChange(values.length <= 1 ? values : values.filter((_, itemIndex) => itemIndex !== index));
  };
  return (
    <div className="space-y-3 rounded-3xl bg-slate-50 p-4">
      <div className="flex items-center justify-between gap-3">
        <div>
          <p className="text-xs font-black uppercase tracking-widest text-slate-500">Call times</p>
          <p className="mt-0.5 text-xs text-slate-400">Pick exact hour and minute for each call window.</p>
        </div>
        <button
          type="button"
          onClick={() => onChange([...values, addThirtyMinutes(values[values.length - 1] || timePlusMinutes(5))])}
      ...
      {values.map((time, index) => (
        <div key={`${index}-${time}`} className="grid grid-cols-[1fr_auto] items-end gap-3 rounded-2xl border border-slate-100 bg-white p-3">
          <TimePicker label={`Time ${index + 1}`} value={time} onChange={(value) => update(index, value)} />
          <button
            type="button"
...
const OutreachScheduler: React.FC<OutreachSchedulerProps> = ({ org, leads, onChanged }) => {
  ...
  const pageScheduleIds = useMemo(() => pagedSchedules.map(getScheduleId), [pagedSchedules]);
  ...
  const handleBulkDelete = async () => {
    ...
    setBusy("bulk-delete");
    try {
      await Promise.all(selectedScheduleIds.map((scheduleId) => voiceCallsApi.outreach.deleteOutreachSchedule(scheduleId)));
        ...
        <div className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
          <div className="mb-5 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            ...
            <div className="flex flex-wrap gap-2">
              <button
                onClick={() => setSelectedScheduleIds((current) => pageScheduleIds.every((id) => current.includes(id)) ? current.filter((id) => !pageScheduleIds.includes(id)) : Array.from(new Set([...current, ...pageScheduleIds])))}
              ...
              {schedules.length > PAGE_SIZE && (
                <div className="mt-5 flex flex-col gap-3 border-t border-slate-100 pt-5 sm:flex-row sm:items-center sm:justify-between">
                  <p className="text-xs font-bold text-slate-400">
                    Showing {(schedulePage - 1) * PAGE_SIZE + 1}-{Math.min(schedulePage * PAGE_SIZE, schedules.length)} of {schedules.length} schedules
                  </p>
                ...
                {callType !== "call_now" && (
                  <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                    ...
                    {callType !== "one_time_batch" && callType !== "custom_rule" ? (
                      <TimePicker label="Time" value={startTime} onChange={(value) => { setStartTime(value); setPreview(null); }} />
                    ) : (
                      <div className="md:col-span-3">
                        <TimeListEditor values={startTimes} onChange={(values) => { setStartTimes(values); setPreview(null); }} />
                      </div>
                    )}
                  </div>
                )}
        ...
        {selectedSchedule && (
          <div className="space-y-4">
            <div className="grid grid-cols-1 gap-3 text-sm md:grid-cols-2">
              <div className="rounded-2xl bg-slate-50 p-4"><p className="text-[10px] font-black uppercase tracking-widest text-slate-400">From</p><p className="mt-1 font-bold text-slate-900">{selectedSchedule.fromNumber || selectedSchedule.from_number || "—"}</p></div>
              <div className="rounded-2xl bg-slate-50 p-4"><p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Timezone</p><p className="mt-1 font-bold text-slate-900">{selectedSchedule.timezone || "—"}</p></div>
              <div className="rounded-2xl bg-slate-50 p-4 md:col-span-2"><p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Purpose</p><p className="mt-1 font-medium text-slate-700">{selectedSchedule.callPurpose || selectedSchedule.call_purpose || "—"}</p></div>
      ...
      <AppModal
        ...
        footer={
          <div className="flex gap-3">
            ...
            >
              {busy === "bulk-delete" || !!busy?.startsWith("delete-") ? "Deleting…" : "Yes, delete permanently"}
            </button>
          </div>
        }
      >
