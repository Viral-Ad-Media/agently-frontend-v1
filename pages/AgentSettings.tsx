          ...
          <div className="lg:col-span-3 bg-white rounded-3xl border border-slate-200 shadow-card p-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5">
              <div>
                ...
                <p className="text-xs text-slate-400 mt-0.5">
                  Create multiple agents and switch the active one.
                </p>
              </div>
                ...
                return (
                  ...
                  >
                    <div className="flex items-start justify-between mb-3">
                      ...
                      {isActive && (
                        <span className="shrink-0 ml-2 rounded-full bg-amber-100 text-amber-700 px-2 py-0.5 text-[9px] font-black uppercase tracking-widest">
                          Live
                        </span>
                      )}
                    </div>
          ...
          <div className="lg:col-span-2 bg-white rounded-3xl border border-slate-200 shadow-card p-6 space-y-5">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              ...
              <div>
                ...
                <p className="text-[10px] text-slate-400 mt-1">
                  Manage in <strong>Phone Numbers</strong> section
                </p>
              </div>
            </div>
            ...
            <div>
              ...
              <p className="text-xs text-slate-400 mt-1 italic">
                Tip: Mention your company name in the first sentence.
              </p>
            </div>
                ...
                <button
                  onClick={async () => {
                    ...
                    try {
                      ...
                      alert(`Call initiated: ${result.callSid}`);
                    } catch (err) {
                      alert(err instanceof Error ? err.message : "Call failed");
                    }
                  }}
                  className="w-full rounded-xl bg-amber-500 text-white py-2.5 text-[10px] font-black uppercase tracking-widest hover:bg-amber-600 active:scale-95 transition-all mt-3"
                >
                  <i className="fa-sharp fa-solid fa-phone-plus mr-2" /> Make
                  Outbound Call
                </button>
              </div>
            </div>
          ...
          <div className="bg-white rounded-3xl border border-slate-200 shadow-card p-6">
            <div className="flex items-start justify-between mb-5">
              <div>
                ...
                <p className="text-xs text-slate-400 mt-0.5">
                  Fetch your site's HTML, convert it to knowledge chunks, and
            ...
            {scrapeResult && (
              <div
                className={`rounded-2xl px-4 py-3 text-xs font-medium flex items-start gap-2 ${scrapeStatus === "error" ? "bg-red-50 text-red-600 border border-red-100" : "bg-emerald-50 text-emerald-700 border border-emerald-100"}`}
              >
                {scrapeStatus === "done" ? (
                  <svg
                    width="14"
              ...
              {[
                {
                  icon: "🔍",
                  label: "Fetches",
                  desc: "HTML from your public pages via Jina.ai",
                },
                {
                  icon: "✂️",
                  label: "Chunks",
                  desc: "Splits content into searchable knowledge blocks",
                },
                {
                  icon: "🗄️",
                  label: "Stores",
                  desc: "Saves chunks to Supabase knowledge_chunks table",
                },
              ].map((item) => (
                <div
                  key={item.label}
          ...
          <div className="bg-white rounded-3xl border border-slate-200 shadow-card p-6">
            <div className="flex items-center justify-between mb-5">
              <div>
                ...
                <p className="text-xs text-slate-400 mt-0.5">
                  Short Q&A pairs the AI uses to answer callers. Max 3–4 visible
                  at once — scroll for more.
                </p>
              </div>
              <div className="flex gap-2">
                <button
                  onClick={() =>
                    void run(
                      "sync",
                      () => onSyncFaqs(scrapeUrl || org.profile.website),
                      "FAQs regenerated",
                    )
                  }
                  disabled={busy === "sync"}
                ...
                <button
                  onClick={() => void run("add-faq", onAddFaq, "FAQ added")}
            ...
            {draft.faqs.length === 0 ? (
              <div className="rounded-2xl border-2 border-dashed border-slate-200 py-12 text-center">
                ...
                <p className="text-xs text-slate-300 mt-1">
                  Import a website or click + Add to create the first entry.
                </p>
              </div>
            ) : (
              <>
                ...
                <div
                  ref={faqScrollRef}
                  className="overflow-x-auto pb-3 -mx-1 px-1 custom-scrollbar"
                >
                  <div
                ...
                <p className="text-[10px] text-slate-400 text-center mt-3">
                  ← scroll to see all {draft.faqs.length} entries →
                </p>
              </>
            )}
